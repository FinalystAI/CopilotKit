...
<Callout type="info">
You can find all `@finalyst/*` packages under the `packages` folder of the monorepo.
</Callout>
...
We have an `examples` folder in the monorepo with a few different examples using CopilotKit. You can run these examples to test your changes, as they are linked to the `@finalyst/*` packages in the monorepo.
