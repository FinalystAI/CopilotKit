...
<Steps>
  ...
  <Step>
    ...
    import { useCoAgent } from "@finalyst/react-core"; // [!code highlight]
...
For this example, you'll need to install the `@finalyst/runtime-client-gql` and `@finalyst/shared` packages.

```bash
pnpm add @finalyst/runtime-client-gql
pnpm add @finalyst/shared
```

```tsx title="ui/app/page.tsx"
import { useCoAgent } from "@finalyst/react-core";
import { TextMessage, MessageRole } from "@finalyst/runtime-client-gql";  // [!code highlight]
import type { Message } from "@finalyst/shared";
