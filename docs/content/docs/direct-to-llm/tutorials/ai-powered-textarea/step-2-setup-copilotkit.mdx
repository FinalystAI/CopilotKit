...
- `@finalyst/react-textarea`: The textarea component for CopilotKit, which enables you to get instant context-aware autocompletions in your app.
...
import { CopilotKit } from "@finalyst/react-core"; // [!code highlight]
...
- First, we imported the `CopilotKit` provider from `@finalyst/react-core`.
- Then, we wrapped the page with the `<CopilotKit>` provider.
- We imported the built-in styles from `@finalyst/react-textarea`.
